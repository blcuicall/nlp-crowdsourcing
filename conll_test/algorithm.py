...
class Manager:
    ...
    def plot_regret(self, regret_history=None, cumulative=True):
        ...
        plt.ylabel('Regret')
        plt.savefig(f'out/pics/conll/{self.run_name}.eps', dpi=600)
        plt.show()

        with open(f'out/regret_logs/conll/{self.run_name}.txt', 'w') as out:
            for step in history:
                out.write(f'{step}\n')
    ...
    def dump_annotations(self, annotations=None, annotation_workers=None):
        ...
        worker_name2sent_tags: dict[str, dict[str, list[str]]] = defaultdict(dict)

        for sent, workers in sent2workers.items():
            for i, worker in enumerate(workers):
                worker_name2sent_tags[f'worker-{worker.id}'][f'sent-{sent}'] = sent2annotations[sent][i]

        if os.path.exists('out/annotations/conll/mturk/extracted_data'):
            shutil.rmtree('out/annotations/conll/mturk/extracted_data')
        for worker_name, sent2tags in worker_name2sent_tags.items():
            worker_dir = os.path.join('out/annotations/conll/mturk/extracted_data', worker_name)
            os.makedirs(worker_dir, exist_ok=True)
            for sent, tags in sent2tags.items():
                with open(os.path.join(worker_dir, f'{sent}.txt'), 'w') as writer:
                    writer.write('\n')
                    for i, tag in enumerate(tags):
                        writer.write(f'{self.utils.sentences[int(sent.removeprefix("sent-"))][i]} {tag}\n')
                    writer.write('\n')

        if os.path.exists('out/annotations/conll/mturk/ground_truth'):
            shutil.rmtree('out/annotations/conll/mturk/ground_truth')
        os.mkdir('out/annotations/conll/mturk/ground_truth')
        for sent, tags in self.utils.sent2silver_tags.items():
            with open(os.path.join('out/annotations/conll/mturk/ground_truth', f'sent-{sent}.txt'), 'w') as writer:
                writer.write('\n')
                for i, tag in enumerate(tags):
                    writer.write(f'{self.utils.sentences[sent][i]} {tag}\n')
                writer.write('\n')

    def get_mv_tags(self, sentence_id, given_annotations=None):
        annotations = given_annotations if given_annotations is not None else self.annotations[sentence_id]
        mv_tags = []
    ...
    def dump_annotations4crowd_layer(self, annotations=None, annotation_workers=None, annotation_eval_types=None):
        sent2annotations = annotations if annotations is not None else self.annotations
        sent2workers = annotation_workers if annotation_workers is not None else self.annotation_workers
        sent2eval_type = annotation_eval_types if annotation_eval_types is not None else self.sent2eval_type

        out_dir = 'out/annotations/conll/ner-mturk/'
        with open(os.path.join(out_dir, 'answers.txt'), 'w') as answers_writer, \
            open(os.path.join(out_dir, 'mv.txt'), 'w') as mv_writer, \
            open(os.path.join(out_dir, 'eval_types.txt'), 'w') as et_writer, \
            open(os.path.join(out_dir, 'ground_truth.txt'), 'w') as gold_writer:
            for sent, annotations in sent2annotations.items():
                answers_sent_lines = [[word] + ['?'] * self.worker_num for word in self.utils.sentences[sent]]
                for i, tags in enumerate(annotations):
                    for j, tag in enumerate(tags):
                        answers_sent_lines[j][sent2workers[sent][i].id + 1] = tag

                for line in answers_sent_lines:
                    answers_writer.write(f'{" ".join(line)}\n')
                answers_writer.write('\n')

                mv_sent_lines = [[word] for word in self.utils.sentences[sent]]
                for i, tag in enumerate(self.get_mv_tags(sent, annotations)):
                    mv_sent_lines[i].append(tag)

                for line in mv_sent_lines:
                    mv_writer.write(f'{" ".join(line)}\n')
                mv_writer.write('\n')

                et_writer.write(f'{sent2eval_type[sent]}\n')

                for i, tag in enumerate(self.utils.sent2silver_tags[sent]):
                    gold_writer.write(f'{self.utils.sentences[sent][i]} {tag}\n')
                gold_writer.write('\n')
            ...
            # answers_writer.write('\n')
            # mv_writer.write('\n')
        # with open(os.path.join(out_dir, 'ground_truth.txt'), 'w') as writer:
        #     for sent, tags in self.utils.sent2silver_tags.items():
        #         for i, tag in enumerate(tags):
        #             writer.write(f'{self.utils.sentences[sent][i]} {tag}\n')
        #         writer.write('\n')
...
class KappaAggManager(MVExpertManager):
    ...
    def __init__(self, workers: list[Worker], utils: Utils, agg_method: str = 'mv', fleiss_kappa_threshold: float = 0.5,
        ...
        self.fleiss_kappa_thresold = fleiss_kappa_threshold
    ...
    def run_name(self):
        return f'{self.__class__.__name__}' \
               ...
               f'-aps={self.utils.annotation_num_per_sentence}' \
               f'-wn={self.selected_worker_num}' \
               f'-kt={self.fleiss_kappa_thresold}' \
               f'-us={self.ucb_scale}'
        ...
        for worker in selected_workers:
            ...
            for sentence_id, annotation, is_fake_annotation in zip(sentence_ids, annotations, is_fake_annotations):
                ...
                if len(self.annotations[sentence_id]) == voter_num:
                    ...
                    if fleiss_kappa < self.fleiss_kappa_thresold:
...
class MVManager(Manager):
    ...
    def get_mv_tags(self, sentence_id, **kwargs):
        mv_tags = []
        votes = [{} for _ in self.utils.sent2silver_tags[sentence_id]]
        for annotation in self.annotations[sentence_id]:
            for i, tag in enumerate(annotation):
...
def run_average(average_over: int = 100,
                ...
                epsilon: float = 0.5,
                selected_worker_num: int = 20) -> NoReturn:
    utils = None
    ...
    for average_round in trange(average_over, desc=f'Averaging over {average_over} trials: ', position=0, ncols=80):
        ...
        if issubclass(manager_class, KappaAggManager):
            manager.fleiss_kappa_thresold = fleiss_kappa_threshold
        ...
        # round_sent2eval_types.append(manager.sent2eval_type)
    ...
    logf = partial(log, filepath=f'out/std_logs/conll/{manager.run_name}.txt')
    with open(f'out/std_logs/conll/{manager.run_name}.txt', 'w') as _:
        pass
    ...
    logf('Actual Scores')
    logf(table)
    with open(f'out/f1_logs/conll/{manager.run_name}.txt', 'w') as out:
    ...
    if manager_class == MVExpertManager:
        ...
        with open(f'out/expct_logs/conll/{manager.run_name}.txt', 'w') as out:
        ...
        # manager.dump_annotations4crowd_layer(annotations=round_sent2annotations[min_greater_idx],
        #                                      annotation_workers=round_sent2workers[min_greater_idx],
        #                                      annotation_eval_types=round_sent2eval_types[min_greater_idx])
        # logf(f'dump:f{span_proportional_f1s[min_greater_idx]:.02f}-m{np.mean(span_proportional_f1s):.02f}')
