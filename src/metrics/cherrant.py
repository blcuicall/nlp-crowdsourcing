...
def cherrant_scores(hyp_m2: list[str], ref_m2: list[str]) -> tuple[float, float, float]:
    """
    Calculate the P, R, and F_0.5 score for GEC tasks.
    :param hyp_m2: list of sentences in hypothesis, each sentence is a string
    Example: hyp_m2 = [
    ...
    :param ref_m2: list of sentences in reference , each sentence is a string
    Example: ref_m2 = [
        "S 冬 阴 功 是 泰 国 最 著 名 的 菜 之 一 ， 它 虽 然 不 是 很 豪 华 ， 但 它 的 味 确 实 让 人 上 瘾 ， 做 法 也 不 难 、 不 复 杂 。\nT0-A0 冬 阴 功 是 泰 国 最 著 名 的 菜 之 一 ， 它 虽 然 不 是 很 豪 华 ， 但 它 的 味 道 确 实 让 人 上 瘾 ， 做 法 也 不 难 、 不 复 杂 。\nA 27 27|||M|||道|||REQUIRED|||-NONE-|||0",
        "S 首 先 ， 我 们 得 准 备 ： 大 虾 六 到 九 只 、 盐 一 茶 匙 、 已 搾 好 的 柠 檬 汁 三 汤 匙 、 泰 国 柠 檬 叶 三 叶 、 柠 檬 香 草 一 根 、 鱼 酱 两 汤 匙 、 辣 椒 6 粒 ， 纯 净 水 4 量 杯 、 香 菜 半 量 杯 和 草 菇 1 0 个 。\nT0 没有错误\nA -1 -1|||noop|||-NONE-|||REQUIRED|||-NONE-|||0"
    ]
    :param beta: for GEC tasks, beta=0.5 is the most common setting
    :returns precision, recall, f_0.5
    """
    # Parse command line args
    args = DefaultArguments()

    # Make sure they have the same number of sentences
    if len(hyp_m2) != len(ref_m2):
        raise ValueError(f'The lengths of {hyp_m2} ({len(hyp_m2)}) and {ref_m2} ({len(ref_m2)}) mismatch.')
    ...
    for sent_id, sent in enumerate(sents):
        ...
        if args.reference_num is None or len(ref_dict.keys()) == args.reference_num:
            ...
            count_dict, cat_dict = evaluate_edits(src, hyp_dict, ref_dict, best_dict, sent_id, args)
