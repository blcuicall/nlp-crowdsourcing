...
class Metrics:
    ...
    def f1_score(self, y_true: list[list[str]], y_pred: list[list[str]], metrics_type: Optional[str] = None):
        ...
        if metrics_type == 'span_exact':
            f1 = seq_f1_score(y_true, y_pred, average='macro', zero_division=0)
        elif metrics_type == 'span_proportional':
            f1 = self.span_proportional_scores(y_true, y_pred)['F1']
        elif metrics_type == 'token':
            f1 = skl_f1_score(self.to_skl_format(y_true), self.to_skl_format(y_pred), average='macro', zero_division=0)
        elif metrics_type == 'pearson':
            f1 = self.span_proportional_scores(y_true, y_pred)['F1']
        else:
            f1 = None
        return f1
    ...
    def precision_score(self, y_true: list[list[str]], y_pred: list[list[str]], metrics_type: Optional[str] = None):
        if not metrics_type:
            metrics_type = self.metrics_type
        if metrics_type == 'span_exact':
            precision = seq_precision_score(y_true, y_pred, average='macro', zero_division=0)
        elif metrics_type == 'span_proportional':
            precision = self.span_proportional_scores(y_true, y_pred)['precision']
        elif metrics_type == 'token':
            precision = skl_precision_score(self.to_skl_format(y_true), self.to_skl_format(y_pred), average='macro', zero_division=0)
        elif metrics_type == 'pearson':
            precision = self.span_proportional_scores(y_true, y_pred)['precision']
        else:
            precision = None
    ...
    def recall_score(self, y_true: list[list[str]], y_pred: list[list[str]], metrics_type: Optional[str] = None):
        if not metrics_type:
            metrics_type = self.metrics_type
        if metrics_type == 'span_exact':
            recall = seq_recall_score(y_true, y_pred, average='macro', zero_division=0)
        elif metrics_type == 'span_proportional':
            recall = self.span_proportional_scores(y_true, y_pred)['recall']
        elif metrics_type == 'token':
            recall = skl_recall_score(self.to_skl_format(y_true), self.to_skl_format(y_pred), average='macro', zero_division=0)
        elif metrics_type == 'pearson':
            recall = self.span_proportional_scores(y_true, y_pred)['recall']
        else:
            recall = None
    ...
    def pearson_corr_coef(self, y_true: list[list[str]], y_pred: list[list[str]]):
        pccs = []
        for tags_true, tags_pred in zip(y_true, y_pred):
            vector_true = [self.tag2value[tag] for tag in tags_true] + [0.000001] # Avoid 0/0
            vector_pred = [self.tag2value[tag] for tag in tags_pred] + [0.000001] # Avoid 0/0
            # print(f'true: {vector_true}')
            # print(f'pred: {vector_pred}')
            pccs.append(np.corrcoef(vector_true, vector_pred)[0, 1])
        return np.mean(pccs)

    def fleiss_kappa(self, worker_tags: list[list[str]]) -> float:
        """
        Calculate the Fleiss Kappa of several(>=2) workers' annotations.
        ...
        # init_input = [[
        #         self.tag2value[worker_tags[j][i]]
        #         for j in range(len(worker_tags))]
        #     for i in range(len(worker_tags[0]))]
        # agged_input = inter_rater.aggregate_raters(init_input)
        # return inter_rater.fleiss_kappa(agged_input[0])

    def cohens_kappa(self, worker_tags: list[list[str]]) -> float:
        """
        Calculate the Cohen's Kappa of two workers' annotations.
        Example of worker_tags:
        [
            ['B-POS', 'I-POS', 'O'    ], # worker 1
            ['B-POS', 'O',     'O'    ], # worker 2
    ...
    # def cohens_kappa(self, y_true: list[list[str]], y_pred: list[list[str]]):
    #     scores = []
    #     for y_true_line, y_pred_line in zip(y_true, y_pred):
    #         y_true_idx = np.array([self.tag2value[tag] for tag in y_true_line])
    #         y_pred_idx = np.array([self.tag2value[tag] for tag in y_pred_line])
    ...
    #             return score, result
    #         score, res = kappa(y_true_idx, y_pred_idx)
    #         scores.append(score)
